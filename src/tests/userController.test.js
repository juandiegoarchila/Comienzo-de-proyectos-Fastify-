...
describe("User Controller", function () {
  ...
  after(async () => {
   ...
   // Dentro del bloque after, ajusta la creación de la tabla:
const table = new Table({
  head: [
    chalk.bold('#'), // Columna para el índice
    chalk.bold('Prueba'),
    chalk.bold('Descripción'),
    chalk.bold('Estado'),
    chalk.bold('Tiempo'),
  ],
  colWidths: [5, 25, 40, 15, 15], // Ajusta el ancho de las columnas
...
testResults.forEach((test, index) => {
  ...
  table.push([
    ...
    test.description.split(' - ')[0], // Prueba
    ...
    console.log(
      chalk.bold.yellowBright(`⏱️ TIEMPO PROMEDIO: ${averageTime}ms`),
    );
    console.log(chalk.bold.blue(`═══════════════════════════════════════\n`));
  });
  ...
  it("[POST] /api/users - Crear usuario con datos inválidos", async function () {
    await runTest(
      "[POST] /api/users - Crear usuario con datos inválidos",
      async () => {
  ...
  it("[POST] /api/users - Crear usuario con email inválido", async function () {
    await runTest(
      "[POST] /api/users - Crear usuario con email inválido",
      async () => {
